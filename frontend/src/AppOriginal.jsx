...
function App() {
  ...
  const [availablePlaceIds, setAvailablePlaceIds] = useState([]);
  ...
  const handleSearch = async () => {
    if (!user) {
      setError('Debes iniciar sesión para hacer búsquedas');
      navigate('/login');
      return;
    }
    ...
    if (!city || !country) {
      setError('Debes ingresar ciudad y país para la búsqueda');
      return;
    }
    ...
    try {
      ...
      setAvailablePlaceIds(data.placeIds || []);
      setSearchMetadata(data);
      
      console.log(`Búsqueda completada: ${data.placeIds?.length || 0} lugares disponibles`);
      
      // Cargar automáticamente los primeros 20
      if (data.placeIds && data.placeIds.length > 0) {
        const loadedResults = await loadMoreResults(data.placeIds, 0);
        
        // Solo cambiar a vista de resultados si se cargaron lugares
        if (loadedResults && loadedResults > 0) {
          setShowSearch(false);
        } else {
          setError('No se encontraron resultados con información de contacto');
        }
      } else {
        setError('No se encontraron resultados nuevos');
      }
  ...
  const loadMoreResults = async (placeIds = availablePlaceIds, startIndex = loadedCount, targetCount = 20) => {
    ...
    try {
      ...
      console.log(`Carga completada: ${totalLoaded} lugares con contacto`);
  ...
  return (
    <div className="app">
      ...
      {showSearch && user && userProfile && (
        <div style={{ position: 'fixed', top: '20px', right: '20px', display: 'flex', gap: '15px', alignItems: 'center', zIndex: 1000 }}>
          {userProfile.plan !== 'free' && (
            <a
              href="https://wa.me/573003501654?text=Hola!%20Necesito%20soporte%20con%20ProspectAI"
              target="_blank"
              ...
              {user && userProfile && (
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '20px', flexWrap: 'wrap' }}>
                    ...
                    <span>•</span>
                    <span>{userProfile.plan === 'free' ? 'Plan Gratis' : 'Plan Premium'}</span>
                    <span>•</span>
                    <span>{userProfile.results_shown} / {userProfile.total_results_limit} resultados</span>
                    {(userProfile.plan === 'free' || userProfile.results_shown >= userProfile.total_results_limit) && (
                      <>
                        <span>•</span>
                        <a
                        ...
                        >
                          {userProfile.plan === 'free' ? 'Activar Premium' : 'Aumentar Límite'}
                        </a>
                      </>
                    )}
                  </div>
        ...
        {!showSearch && results.length > 0 && (
          <>
            <div className="layout-container">
              ...
              {loadedCount < availablePlaceIds.length && (
                <div className="load-more">
                  ...
                  >
                    {loadingMore ? 'Cargando más...' : `Cargar más resultados (${availablePlaceIds.length - loadedCount} disponibles)`}
                  </button>
                </div>
              )}
                </div>
